...
# adapted from: https://github.com/matrix-org/sytest/blob/release-v1.119/tests/50federation/30room-join.pl
...
foreach my $versionprefix ( qw ( v1 v2 ) ) {
   test "Inbound federation can receive $versionprefix /send_join",
      requires => [ $main::OUTBOUND_CLIENT, $main::INBOUND_SERVER,
                    local_user_and_room_fixtures( room_opts => { room_version => "10" } ),
                    federation_user_id_fixture() ],
...
test "Inbound federation rejects attempts to join v1 rooms from servers without v1 support",
   ...
   do => sub {
      ...
      ->then( sub {
         ...
         assert_eq( $body->{room_version}, "10", 'room_version' );
         Future->done( 1 );
      });
   };
...
test "Inbound: send_join rejects invalid JSON for room version 6",
   ...
   do => sub {
      ...
      $outbound_client->do_request_json(
         ...
         $datastore->sign_event( \%event );
         my $event_id = SyTest::Federation::Protocol::id_for_event(\%event, '10');
