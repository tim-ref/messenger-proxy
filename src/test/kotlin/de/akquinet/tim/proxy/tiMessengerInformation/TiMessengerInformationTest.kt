...
class TiMessengerInformationTest :
  FunSpec({
    val tiMessengerInformationConfig =
      ProxyConfiguration.TiMessengerInformationConfiguration(port = 7777)
    ...
    fun withTiMessengerInformationTestApplication(
      ...
      val apiUnderTest =
        TiMessengerInformationApi(
          tiMessengerInformationConfig,
          TIMessengerInformationService(federationListCache),
          matrixAuthorizationService,
        )

      testApplication {
        routing { with(apiUnderTest) { tiMessengerInformationRoutes() } }
    ...
    context("unauthorized") {
      val dummyMatrixAuthorizationError =
        mockk<MatrixAuthorizationError> { every { message } returns "unauthorized" }

      val userIsNotAuthorizedService =
        mockk<MatrixAuthorizationService> {
    ...
    test("/tim-information/ answers OK with TiMessengerInformation") {
      withTiMessengerInformationTestApplication(
        ...
        val tiMessengerInformation =
          response.bodyAsText().let { Json.decodeFromString<TiMessengerInformation>(it) }
        tiMessengerInformation.title.shouldNotBeBlank()
        tiMessengerInformation.version shouldBe "1.0.0"
      }
    }

    val someFederationListCache =
