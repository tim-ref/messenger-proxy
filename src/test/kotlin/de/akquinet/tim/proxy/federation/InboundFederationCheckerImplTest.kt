...
class InboundFederationCheckerImplTest :
  ShouldSpec({
    ...
    val eventResponseString =
      """{
                                ...
                                """
    val defaultTimAuthorizationCheckConfiguration =
    ...
    fun withCut(
      ...
      timAuthorizationCheckConfiguration: ProxyConfiguration.TimAuthorizationCheckConfiguration =
        defaultTimAuthorizationCheckConfiguration,
        ...
        externalServices {
          hosts(destinationUrl) {
            routing {
              ...
              put("/_matrix/federation/v2/invite/{roomId}/{eventId}") { _ ->
                // horribly complicated event structure omitted for testing
                call.respondText("{}", Application.Json)
              }
              put("/_matrix/federation/v2/send_join/{roomId}/{eventId}") { _ ->
                call.respondText("{}", Application.Json)
              }
              get("/_matrix/federation/v1/make_join/{roomId}/{userId}") { _ ->
                call.respondText("{}", Application.Json)
              }
              get("/_matrix/client/v3/publicRooms") { _ ->
                call.respondText(contentType = Application.Json, status = OK, text = publicRooms)
              }
              ...
              get("/_synapse/admin/v1/rooms/{roomId}") { _ ->
                /// Use roomId opaqueId to choose joinRules setting
                val roomIdPathParam =
                  call.parameters["roomId"] ?: throw BadRequestException("roomId is missing")
                val roomId = RoomId(roomIdPathParam)

                val joinRuleStrings =
                  listOf("public", "knock", "restricted", "knock_restricted", "private")
    ...
    context("authorization needed") {
      should("forward federated domain") {
        withCut {
          federationListCacheMock.domains.value = setOf(insuranceDomainFed())
      ...
      /**
       * A_26331 Der TI-M Fachdienst MUSS nicht authentisierte Requests zum Endpunkt
       * /_matrix/federation/v1/version mit einer HTTP 401 Response ablehnen.
       */
      ...
      should("forward authenticated request for server version") {
        withCut {
          ...
          val response =
            client.get("/_matrix/federation/v1/version") { matrixAuthorizationHeader() }
    ...
    context("federation invite") {
      // gemSpec_TI-Messenger-Dienst_V1.1.1, 3.5.2.2+: after federation check, check invite
      // permission on proxy
      should("fail if blocked by invite permission") {
        withCut {
          federationListCacheMock.domains.value = setOf(insuranceDomainFed())
      ...
      should("succeed for valid invite permission") {
        withCut {
          federationListCacheMock.domains.value = setOf(insuranceDomainFed())
          coEvery { synapseAdminAPIValidator.validateInvitePermission(any(), any()) } returns
      ...
      // gemSpec_TI-M_Basis_V1.0.0, AFO_25046: after federation check, send invite to client to
      // check invite permission

      should("succeed if invite permission check on client") {
        val clientTimAuthorizationCheckConfiguration =
          ProxyConfiguration.TimAuthorizationCheckConfiguration(
...
private fun insuranceDomainFed() =
  FederationList.FederationDomain(domain = "fed", isInsurance = true, telematikID = "telematik")
