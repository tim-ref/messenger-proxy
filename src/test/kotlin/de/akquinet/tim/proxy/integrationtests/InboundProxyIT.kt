...
class InboundProxyIT {
  ...
  private val pushRuleWithoutTemplateEndpoint =
    "$proxy/_matrix/client/v3/pushrules/global/not_a_template/foo"
  ...
  private val stateEndpoint = "/_matrix/client/v3/rooms/room1%3Asynapse/state/m.room.join_rules"
  ...
  private val logInfoConfig =
    ProxyConfiguration.LogInfoConfig("rawdata/path", "doctor", "2384234234", "MP-1", "home.de")

  private lateinit var inboundApplicationEngine: ApplicationEngine

  private var timAuthorizationCheckConfiguration =
  ...
  // These tests verify the changes made in the CustomMatrixServer.kt in comparison to the original
  // MatrixApiServer.kt
  @Test
  fun `should return NotFound on not-existent route`() = runTest {
    ...
    response.bodyAsText() shouldEqualJsonMatrixStandard
      ErrorResponse(errcode = "M_NOT_FOUND", error = "unsupported (or unknown) endpoint")
  }
  ...
  fun shouldReturnBadRequestOnBadRequestException() = runTest {
    ...
    response.bodyAsText() shouldEqualJsonMatrixStandard
      ErrorResponse(
        errcode = "M_UNKNOWN",
        error =
          "net.folivo.trixnity.core.model.push.PushRuleKind does not contain element with name 'not_a_template'",
      )
  }
  ...
  fun shouldReturnBadRequestForRequestToGetToken() = runTest {
    ...
    response.bodyAsText() shouldEqualJsonMatrixStandard
      ErrorResponse(errcode = "M_NOT_FOUND", error = "No resource was found for this request.")
  }

  @Test
  fun shouldReturnForbiddenForGuestRegister() = runTest {
    val response =
      httpClient.post(registerAsGuestEndpoint) {
  ...
  @Test
  fun shouldReturnForbiddenForPublicRoomPostRequest() = runTest {
    val response =
      httpClient.post("$proxy/_matrix/federation/v1/publicRooms") {
        headers {
  ...
  @Test
  fun shouldReturnForbiddenForPublicRoomGetRequest() = runTest {
    val response =
      httpClient.get("$proxy/_matrix/federation/v1/publicRooms") {
        headers {
          append(HttpHeaders.ContentType, "application/json")
          append(
            HttpHeaders.Authorization,
  ...
  @Test
  fun shouldRejectRequestsSettingPublicJoinRuleState() = runTest {
      ...
      assertSoftly {
        ...
        response.bodyAsText() shouldEqualJson
          """{
                      ...
                      "error": "Subsequent creation of public rooms not permitted"
                  }"""
      }
    }
  }
  ...
  @Test
  fun shouldAcceptRequestsSettingPrivateJoinRuleState() = runTest {
  ...
  fun shouldNotFindPreviewUrlEndpoint() = runTest {
    testApplication {
      proxyWithClientServerRoutes(defaultConfig(httpClient = client))
      homeserverWithRouting {
        get("/_matrix/client/v1/media/preview_url") { _ -> call.respondText("Hi!") }
      }
  ...
  fun shouldNotFindLegacyPreviewUrlEndpoint() = runTest {
    testApplication {
      proxyWithClientServerRoutes(defaultConfig(httpClient = client))
      homeserverWithRouting {
        get("/_matrix/media/v3/preview_url") { _ -> call.respondText("Hi!") }
      }
