...
fun ApplicationTestBuilder.proxyWithClientServerRoutes(inboundClientRoutes: InboundClientRoutes) {
  application {
    ...
    routing { with(inboundClientRoutes) { clientServerApiRoutes() } }
  }
}

/** Adds a module containing the proxy's Matrix server-server API routes to TestApplication. */
fun ApplicationTestBuilder.inboundProxyWithServerServerRoutes(
  inboundFederationRoutes: InboundFederationRoutes
) {
  application {
    installCustomMatrixApiServer(Json)
    routing { with(inboundFederationRoutes) { serverServerApiRoutes() } }
  }
}

/** Adds a module containing the proxy's Matrix server-server API routes to TestApplication. */
fun ApplicationTestBuilder.outboundProxyWithServerServerRoutes(
  outboundFederationRoutes: OutboundFederationRoutes
) {
  application {
    installCustomMatrixApiServer(Json)
    routing { with(outboundFederationRoutes) { serverServerApiRoutes() } }
  }
}
...
fun defaultConfig(
  ...
  logConfiguration: ProxyConfiguration.LogInfoConfig =
  ...
  berechtigungsstufeEinsService: BerechtigungsstufeEinsService =
    BerechtigungsstufeEinsService(FederationListCacheMock()),
...
/**
 * Builds mock Matrix homeserver, installs a Routing plugin for this Application and runs a
 * configuration script on it.
 */
