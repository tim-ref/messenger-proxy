...
class MatrixOpenIdClientTest :
  ShouldSpec({
    should("get authenticatedUser") {
      val matrixAuthClient = matrixOpenIdClient("@mxid:server.example.com")
      matrixAuthClient.authenticatedUser("TOKEN") shouldBe
        UserAuthenticationResult.Success(token = "TOKEN", mxid = "@mxid:server.example.com")
    }
    ...
    should("use token") {
      val matrixAuthClient =
        matrixOpenIdClient(
          verifyUrl = {
            it.fullPath shouldEndWith
              "/_matrix/federation/v1/openid/userinfo?access_token=ACCESS_TOKEN"
            it.hostWithPort shouldBe "localhost:7070"
          }
        )
      matrixAuthClient.authenticatedUser("ACCESS_TOKEN") shouldBe
        UserAuthenticationResult.Success(token = "ACCESS_TOKEN", mxid = "")
    }
  })
...
private fun matrixOpenIdClient(
  ...
  return MatrixOpenIdClient(mockClient, aProxyConfig("http://localhost:7070"))
}

fun aProxyConfig(homeserver: String) =
