...
class MatrixAuthorizationServiceImplTest :
  FunSpec({
    ...
    context("authorize") {
      test("authorize should fail when mxid is missing") {
        ...
        matrixAuthorizationService.authorize(headers) shouldBeLeft
      ...
      test("authorize should fail when authorization header is malformed") {
        val headers: Headers =
          headersOf(
            ...
            Pair("authorization", listOf("none")),
          )
        matrixAuthorizationService.authorize(headers) shouldBeLeft
          MatrixAuthorizationError.MalformedBearerToken("none")
      }
      ...
      test("authorize with actual authorization header should pass") {
        ...
        coEvery { authClient.authenticatedUser(any()) } returns
          UserAuthenticationResult.Success("TOKENTOKENTOKEN", "@mxid:server.example.com")
      ...
      test("authorize should fail when openid auth fails") {
        ...
        coEvery { authClient.authenticatedUser(any()) } returns
          UserAuthenticationResult.Failure("TOKENTOKENTOKEN")

        matrixAuthorizationService.authorize(headers) shouldBeLeft
          MatrixAuthorizationError.AuthenticationFailed("TOKENTOKENTOKEN")
      }
    }
    ...
    context("authorizeWithoutMxid") {
      test("authorizeWithoutMxid should fail when authorization header is missing") {
        ...
        matrixAuthorizationService.authorizeWithoutMxid(headers) shouldBeLeft
          MatrixAuthorizationError.MissingAuthorizationHeader
      }

      test(
        "authorizeWithoutMxid should fail when the authorization header does not match the expected format"
      ) {
      ...
      test(
        "authorizeWithoutMxid should succeed with a valid authorization header and no mxid header"
      ) {
      ...
      test(
        "authorizeWithoutMxid should succeed with a valid authorization header even when an extraneous mxid header is present"
      ) {
        val headers =
          headersOf(
            Pair("mxId", listOf("@mxid:server.example.com")),
