...
@Serializable
data class HealthLivenessResponse(val inboundProxy: Boolean, val outboundProxy: Boolean)

@Serializable class HealthReadinessResponse
...
class ActuatorRoutesImpl(
  ...
  override suspend fun start(): ApplicationEngine =
    embeddedServer(Netty, port = this@ActuatorRoutesImpl.actuatorConfig.port) {
        ...
        routing {
          ...
          // Liveness probe
          get("$healthPath/liveness") { performHealthCheck(healthPath) }

          // Readiness probe
          get("$healthPath/readiness") { handleReadinessRequest() }

          // logger actuator
          get("$loggingPath/levels") { performLoggerActuator() }

          val paramNewLogLevel = "newLogLevel"
          put("$loggingPath/{$paramNewLogLevel}") { initiateLogs(paramNewLogLevel) }
  ...
  private fun PipelineContext<Unit, ApplicationCall>.initiateLogs(paramNewLogLevel: String) =
    runBlocking {
      val newLogLevel = call.parameters[paramNewLogLevel].toString()
  ...
  private fun PipelineContext<Unit, ApplicationCall>.performLoggerActuator() = runBlocking {
    val acceptedValues =
      org.slf4j.event.Level.entries.joinToString(", ", "[", "]") { """"${it.name}"""" }
  ...
  private fun PipelineContext<Unit, ApplicationCall>.performHealthCheck(healthPath: String) =
    runBlocking {
