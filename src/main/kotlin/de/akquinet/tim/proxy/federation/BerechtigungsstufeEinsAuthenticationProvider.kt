...
class BerechtigungsstufeEinsAuthenticationProvider
internal constructor(
  ...
  override suspend fun onAuthenticate(context: AuthenticationContext) {
    ...
    val origin =
      (parsedAuthorizationHeader as? HttpAuthHeader.Parameterized)?.let { authHeader ->
        authHeader
          .takeIf { it.authScheme.equals("X-Matrix", ignoreCase = true) }
    ...
    if (config.enforceDomainList && cause != null)
      log.info {
        """authorization failed: url="${call.request.uri}" mode=${config.proxyMode} origin=$origin cause=$cause (authHeader=${call.request.headers[HttpHeaders.Authorization]})"""
      }
    else
      log.trace {
        """url="${call.request.uri}" mode=${config.proxyMode} origin=$origin (authHeader=${call.request.headers[HttpHeaders.Authorization]})"""
      }
          ...
          NoCredentials ->
            challengeCall.respond<ErrorResponse>(
              ...
              ErrorResponse.Unauthorized(
                "missing X-Matrix authorization header with ${config.proxyMode.domainParameterName} field"
              ),
            )
...
fun AuthenticationConfig.berechtigungsstufeEinsCheck(
  ...
  val provider =
    BerechtigungsstufeEinsAuthenticationProvider(
      berechtigungsstufeEinsService = checkerService,
      config =
        BerechtigungsstufeEinsAuthenticationProvider.Config().apply(configure).apply {
          skipWhen {
            it.attributes.getOrNull(withoutAuthAttributeKey) in
              listOf(AuthRequired.NO, AuthRequired.OPTIONAL)
          }
        },
    )
  register(provider)
