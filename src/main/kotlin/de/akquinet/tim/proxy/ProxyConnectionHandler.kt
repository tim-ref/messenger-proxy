...
class ProxyConnectionHandler(private val manager: OutboundProxyCertificateManager) :
  ...
  private fun handleHttpMessage(ctx: ChannelHandlerContext, httpRequest: HttpRequest) {
    if (httpRequest.decoderResult().isFailure) {
      ...
      when (httpRequest.method()) {
        HttpMethod.CONNECT -> handleHttpMethodConnect(httpRequest, ctx)
        else -> {
          log.trace { "got request ${httpRequest.uri()} and inject destination $destination" }
          destination?.also { httpRequest.headers()[HttpHeaders.Host] = destination }
  ...
  private fun handleHttpMethodConnect(httpRequest: HttpRequest, ctx: ChannelHandlerContext) {
